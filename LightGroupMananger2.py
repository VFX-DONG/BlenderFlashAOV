...
class LIGHTGROUP_UL_list(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        for obj in bpy.data.objects:
            if obj.type == 'LIGHT' and obj.lightgroup == group.name:
                if any(col in enabled_collections for col in obj.users_collection):
                    any_light_visible = True
                    break

        # world.lightgroup 也作为“可见性”的参考（不过 world 不受集合影响）
        if not any_light_visible and not (scene.world and getattr(scene.world, "lightgroup", "") == group.name):
            return  # 此组当前不可见，跳过绘制（自动折叠）
        ...
        count = sum(
            ...
            if obj.type == 'LIGHT' and obj.lightgroup == group.name and
            any(col in enabled_collections for col in obj.users_collection)
        )
        if scene.world and getattr(scene.world, "lightgroup", "") == group.name:
            count += 1
...
class LIGHTGROUP_OT_add_to(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for obj in selected_lights:
            # 如果灯光已属于当前组，则移除关联
            if obj.lightgroup == self.group_name:
                obj.lightgroup = ""
            else:
                # 设置灯光组属性
                obj.lightgroup = self.group_name

# 替换现有 context.area.tag_redraw() 为：
...
class LIGHTGROUP_OT_toggle_group(bpy.types.Operator):
    ...
    bl_label = translate("切换组可见性")
    bl_description = translate("切换该灯光组下所有灯光的显示状态")
    group_name: bpy.props.StringProperty()
    ...
    def execute(self, context):
        ...
        # 查找目标组记录
        group_item = next((item for item in scene.lightgroup_list if item.name == self.group_name), None)
        if not group_item:
            self.report({'ERROR'}, translate("未找到灯光组"))
            return {'CANCELLED'}

        # 切换可见性状态
        new_visibility = not group_item.visible
        group_item.visible = new_visibility
        ...
        # 如果是世界环境也属于该组，也同步控制它的 use_nodes 属性为示意（可根据你具体实现修改）
        world = scene.world
        if world and world.get("lightgroup") == self.group_name:
            world.mute = not new_visibility  # 或你已有的控制逻辑
    ...
    def execute(self, context):
        ...
        for obj in context.selected_objects:
            ...
            viewlayer_group_info = next(
                (g for g in scene.viewlayer.lightgroup if g == view_layer.name), None)
            if not viewlayer_group_info:
                viewlayer_group_info = scene.viewlayer.lightgroup_dict.add()
